...
export default function AdminDashboard() {
  ...
  const [recentRoutes, setRecentRoutes] = useState<AdminRoute[]>([])
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [stats, eventsData, routesData] = await Promise.all([
        getDashboardStats(),
        getAdminEvents(1, 5), // Primeros 5 eventos
        getAdminRoutes(1, 5) // Primeras 5 rutas
      ])
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'sold_out': return 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30'
    ...
    <div className="p-6">
      ...
      >
        <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6 hover:bg-gray-800/70 transition-all duration-300">
          ...
          <p className="text-gray-400 text-sm">Crecimiento mensual en ventas</p>
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6 hover:bg-gray-800/70 transition-all duration-300">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-semibold text-white">Rutas Top</h3>
          </div>
          <div className="text-lg font-bold text-cyan-400 mb-2">
            {dashboardData?.popularRoute || 'N/A'}
          </div>
          <p className="text-gray-400 text-sm">Ruta más demandada</p>
          <div className="mt-3 text-sm text-gray-500">
            {dashboardData?.recentBookings || 0} reservas en los últimos 30 días
          </div>
        </div>
