...
const mockEvent: EventDetails = {
  ...
  requires_seats: true, // Conciertos en estadios SÍ necesitan asientos específicos
...
function EventDetailClient({ params }: EventDetailPageProps) {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      try {
        ...
        // Simulate different types of events based on ID
        await new Promise(resolve => setTimeout(resolve, 800))
        
        let simulatedEvent: EventDetails
        
        switch (eventId) {
          case '1': // Concierto con asientos
            simulatedEvent = {
              ...mockEvent,
              id: '1',
              title: 'Concierto de Rock Nacional - Los Saicos',
          ...
          case '3': // Conferencia híbrida (sin asientos específicos)
            simulatedEvent = {
              ...mockEvent,
              id: '3',
          ...
          case '4': // Evento deportivo con asientos
            simulatedEvent = {
              ...mockEvent,
              id: '4',
          ...
          case '5': // Fiesta en club (entrada general)
            simulatedEvent = {
              ...mockEvent,
              id: '5',
              ...
              venues: {
                id: 'venue5',
                name: 'Club Exodus',
                address: 'Av. Larco 1044, Miraflores',
                city: 'Lima',
                capacity: 800
              }
            }
  ...
  const getButtonText = () => {
    ...
    if (event.event_type === 'virtual') {
      return `Comprar Acceso Virtual - S/ ${event.price_from}`
    }

    // Determinar si necesita asientos específicos
    const needsSpecificSeats = (
      event.event_type === 'presential' && 
      event.requires_seats === true &&
      ['concert', 'theater', 'sports'].includes(event.category)
    )

    if (needsSpecificSeats) {
      return 'Comprar Entradas'
    }

    // Para eventos de entrada general
    return `Comprar Entrada General - S/ ${event.price_from}`
  }

  const handleBookNow = () => {
    if (!event) return

    // Para eventos virtuales, verificar autenticación y ir directo al pago
    if (event.event_type === 'virtual') {
      if (!user) {
    ...
    // Determinar si el evento necesita selección de asientos específicos
    // Basado en categoría y tipo de evento
    const needsSpecificSeats = (
      event.event_type === 'presential' && 
      event.requires_seats === true &&
      ['concert', 'theater', 'sports'].includes(event.category)
    ...
    // Para eventos que requieren selección de asientos específicos
    if (needsSpecificSeats) {
      // Ir directamente a la selección de asientos (no verificar auth aquí)
      router.push(`/events/${event.id}/seat-selection`)
      return
    }

    // Para eventos de entrada general, verificar autenticación y ir directo al pago
    if (!user) {
      router.push('/auth/login')
      return
    }
